...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--retain-failed-temp",
        action="store_true",
        help="Keep failed temporary .docx files for local debugging. Default: delete failed temps.",
    )
    parser.add_argument(
        "--no-overwrite-output",
        action="store_true",
        help="Fail if the resolved output .docx already exists. Default: overwrite and report it.",
    )
    return parser
...
def run(
    ...
    *,
    ...
    execution_report = apply_operations_transactional(
        input_file,
        output_docx,
        style_report.operations,
        template=template,
        document_before=detected,
        report_before=style_report,
        retain_failed_temp=retain_failed_temp,
    ...
    payload = {
        ...
        "output": execution_report.output_path if success else None,
        "expected_output": execution_report.expected_output_path or str(output_docx),
        ...
        "temp_output_path": execution_report.temp_output_path,
        "temp_file_retained": execution_report.temp_file_retained,
        "output_existed_before": execution_report.output_existed_before,
        "output_overwritten": execution_report.output_overwritten,
...
def print_summary(payload: dict[str, Any]) -> None:
    ...
    print(f"- Output file: {payload['output'] or '(not generated)'}")
    print(f"- Expected output file: {payload['expected_output']}")
    ...
    print(f"- Execution status: {payload['execution_status']}")
    print(f"- Issues after: {payload['issues_after']}")
    print(f"- Operations after: {payload['operations_after']}")
    print(f"- Output existed before: {payload['output_existed_before']}")
    print(f"- Output overwritten: {payload['output_overwritten']}")
    print(f"- Temp retained: {payload['temp_file_retained']}")
...
def main() -> None:
    ...
    try:
        run(
            ...
            args.template,
            args.output,
            retain_failed_temp=args.retain_failed_temp,
